...
ID_COLOR={1: '#EEE8AA', 2: '#C67C7B', 3: '#FFA500', 4: '#6B8023', 5: '#ADD8E6', 6: '#7BA779', 7: '#E87A90', 8: '#FF8C69', 10: '#F08080', 15: '#727171', 16: '#FFD700', 17: '#D3A2C7'}
...
def fix_nodes(prev_mks, ind_fixed_nodes):
    # print(type(prev_mks))  # For debugging: Check the type of prev_mks
    # Ensure prev_mks is a PyTorch tensor
    if isinstance(prev_mks, np.ndarray):  # If prev_mks is a numpy array
        prev_mks = torch.tensor(prev_mks)  # Convert to PyTorch tensor

    given_masks = prev_mks.clone().detach()  # Clone and detach after conversion
    ...
    given_masks = given_masks.unsqueeze(1)  # Add an extra dimension (e.g., for channels)
...
def _init_input(graph, prev_state=None, mask_size=64):
    ...
    # Ensure given_eds is a tensor, not a NumPy array
    if isinstance(given_eds, np.ndarray):
        given_eds = torch.tensor(given_eds)

    given_eds = given_eds.long().clone().detach()  # Proper conversion, clone, and detach
    ...
    # Initialize masks using the fix_nodes function
...
#     print(type(prev_mks))
#     given_masks = torch.tensor(prev_mks).clone().detach()
#     ind_not_fixed_nodes = torch.tensor([k for k in range(given_masks.shape[0]) if k not in ind_fixed_nodes])
#     ## Set non fixed masks to -1.0
#     given_masks[ind_not_fixed_nodes.long()] = -1.0
...
#     given_eds=given_eds.clone().detach()
#     z = torch.randn(len(given_nds), 128).float()
#     # unpack
#     fixed_nodes = prev_state['fixed_nodes']
#     prev_mks = torch.zeros((given_nds.shape[0], mask_size, mask_size))-1.0 if (prev_state['masks'] is None) else prev_state['masks']
...
# Assuming ID_COLOR is defined elsewhere in your code
# Example: ID_COLOR = {1: "red", 2: "green", 3: "blue", ...}
...
def draw_graph(g_true):
    ...
    colors_H = []
    node_size = []
    edge_color = []  # Ensure edge_color is initialized
    linewidths = []
    edgecolors = []
    ...
    for k, label in enumerate(g_true[0]):
        _type = label + 1
        if _type >= 0 and _type not in [15, 17]:
            G_true.add_node(k, label=k)  # Use add_node instead of add_nodes_from
            colors_H.append(ID_COLOR.get(_type, 'gray'))  # Default to gray if not found in ID_COLOR
            node_size.append(1000)
            edgecolors.append('blue')
            linewidths.append(0.0)
            
    # add outside node
    G_true.add_node(-1, label=-1)
    colors_H.append("white")
    node_size.append(750)
    edgecolors.append('black')
    linewidths.append(3.0)
    ...
    # plt.switch_backend("Agg")    # to resolve the error 
    plt.figure(figsize=(8, 6))  # Optional: Adjust figure size for better layout
    pos = nx.nx_agraph.graphviz_layout(G_true, prog='neato')
    
    nx.draw(
        G_true, pos, node_size=node_size, linewidths=linewidths, node_color=colors_H, font_size=14,
        font_color='white', font_weight='bold', edgecolors=edgecolors, edge_color=edge_color,
        width=4.0, with_labels=False
    )
    
    # Use subplots_adjust if tight_layout is causing issues
    plt.subplots_adjust(left=0.1, right=0.9, top=0.9, bottom=0.1)

    # Save the figure
    plt.savefig('./static/images/_true_graph.jpg', format="jpg")
    plt.close('all')
    
    # Post-process saved image
    rgb_im = Image.open('./static/images/_true_graph.jpg')
    rgb_arr = pad_im(rgb_im).convert('RGBA')
